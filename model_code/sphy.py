...
class sphy(pcrm.DynamicModel):
	...
	def initial(self):
		...
		# caw edit, 2025-10-15, 
		# Initialize tracers for snowmelt and rainfall contributions
		self.RootWater_snow = self.RootWater * 0
		self.RootWater_rain = self.RootWater * 00
		# end CAW edit, 2025-10-15
		...
		# Initialize tracers for snowmelt and rainfall contributions
		self.SubWater_snow = self.SubWater * 0
		self.SubWater_rain = self.SubWater * 0
		...
		if self.GroundFLAG == 1:
			...
			# Ensure groundwater tracers and storage arrays exist
			self.Gw_snow = getattr(self, 'Gw_snow', self.SubWater * 0)
			self.Gw_rain = getattr(self, 'Gw_rain', self.SubWater * 0)
			self.Gw_glac = getattr(self, 'Gw_glac', self.SubWater * 0)
		else:
	...
	def dynamic(self):
		...
		if self.SnowFLAG == 1:
			...
			Rain, self.SnowR,self.SnowSoil,OldTotalSnowStore = self.snow.dynamic(self, pcr, Temp, Precip, Snow_GLAC, ActSnowMelt_GLAC, SnowFrac, RainFrac, SnowR_GLAC)
		else:
			Rain = Precip
			self.SnowR = 0
		...
		self.RootWater_snow += self.SnowSoil  ## CAW track snowmelt contribution
		...
		# Report infiltration corrected for glacier fraction # CAW edit, 2025-10-16
		Infil_report = Infil * (1 - self.GlacFrac)
		self.reporting.reporting(self, pcr, 'InfilF', Infil_report)
		# CAW edit, 2025-10-15, Update rootzone tracers with rainfall infiltration
		self.RootWater_rain = pcr.ifthenelse(RainFrac > 0, self.RootWater_rain + Infil, self.RootWater_rain)
		#-Updated rootwater content
		self.RootWater = pcr.ifthenelse(RainFrac > 0, self.RootWater + Infil, self.RootWater)
		# -Report rootzone weater content snow and rain
		self.reporting.reporting(self, pcr, 'RootWater_snow', self.RootWater_snow)
		self.reporting.reporting(self, pcr, 'RootWater_rain', self.RootWater_rain) #v this is bad, it can go over rootzone reservoir size
		...
		RootWater_total = pcr.max(self.RootWater, 1e-6)

		# Fraction of rootwater from snow and rain
		frac_snow = self.RootWater_snow / RootWater_total
		frac_rain = self.RootWater_rain / RootWater_total

		# Apply fractions to rootzone percolation
		rootperc_snow = frac_snow * rootperc
		rootperc_rain = frac_rain * rootperc
		# Subtract percolated amounts from rootzone tracers
		self.RootWater_snow -= rootperc_snow
		self.RootWater_rain -= rootperc_rain
		# Now subtract total rootperc from RootWater (as in your original code)
		#-Update the RootWater content
		# Roottemp = self.RootWater
		self.RootWater = self.RootWater - (self.RootDrain + rootperc)
		# Report rootzone percolation fractions (optional)
		self.reporting.reporting(self, pcr, 'RootPercSnow', rootperc_snow * (1 - self.GlacFrac))
		self.reporting.reporting(self, pcr, 'RootPercRain', rootperc_rain * (1 - self.GlacFrac))
		# end edit CAW, 2025-10-16 
		...
		# Update subsoil tracers with incoming rootzone percolation CAW edt 2025-10-16
		self.SubWater_snow += rootperc_snow
		self.SubWater_rain += rootperc_rain
		# end caw edit 2025-10-16
		if self.GroundFLAG == 0:
			if self.SeepStatFLAG == 0:
				try:
		...
		if self.GroundFLAG == 1:   # sub percolation will be calculated instead of subdrainage
		    ...
		    # --- NEW: calculate fractions for subsoil percolation --- edit CAW 2025-10-16
			SubWater_total = pcr.max(self.SubWater, 1e-6)  # prevent divide by zero
			frac_sub_snow = self.SubWater_snow / SubWater_total
			frac_sub_rain = self.SubWater_rain / SubWater_total
			
			# Apply fractions to subsoil percolation
			SubPercSnow = frac_sub_snow * subperc
			SubPercRain = frac_sub_rain * subperc

			# Subtract percolated amounts from subsoil tracers
			self.SubWater_snow -= SubPercSnow
			self.SubWater_rain -= SubPercRain
			# in subsoil calculation (earlier in sphy.py)
			#self.SubPercSnow = SubPercSnow
			#self.SubPercRain = SubPercRain
			# --- optional: report percolation fractions ---
			self.reporting.reporting(self, pcr, 'SubPercSnow', SubPercSnow * (1 - self.GlacFrac))
			self.reporting.reporting(self, pcr, 'SubPercRain', SubPercRain * (1 - self.GlacFrac))
			# end edit CAW 2025-10-16
		...
		if self.GroundFLAG == 1:
			...
			self.groundwater.dynamic(self, pcr, ActSubPerc, GlacPerc, SubPercSnow, SubPercRain)
		else:
